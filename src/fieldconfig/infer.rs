...
fn infer_kind_from_col(col: &ColumnSchema) -> FieldKind {
    ...
    if dt == "time" || dt.starts_with("time ") {
        return FieldKind::Time;
    }
    if is_int_type(&dt) {
        return infer_int_by_name(&n);
    }
    if is_float_type(&dt) {
        return FieldKind::Float;
    }
    if is_decimal_type(&dt) {
...
fn infer_int_by_name(n: &str) -> FieldKind {
    if ends_with_any(
        n,
        &[
            "_count", "_num", "_number", "_qty", "_total", "_rank", "_score", "_size", "_order",
            "_seq",
        ],
    ) {
...
fn infer_text_by_name(n: &str) -> FieldKind {
    ...
    if contains_any(n, &["url", "uri", "website", "homepage", "href", "avatar"]) {
    ...
    if n == "ip" || n.ends_with("_ip") || contains_any(n, &["_ip_"]) {
        return FieldKind::Ip;
    }
    if contains_any(n, &["slug"]) {
        return FieldKind::Slug;
    }
    if n == "version" || n.ends_with("_version") {
        return FieldKind::Semver;
    }
    if n == "color" || n == "colour" || n.ends_with("_color") {
        return FieldKind::Color;
    }
    if contains_any(n, &["user_agent", "useragent"]) {
        return FieldKind::UserAgent;
    }
    if contains_any(
        n,
        &[
    ...
    if contains_any(n, &["username", "login"]) {
        return FieldKind::Username;
    }
    if contains_any(
        n,
        &[
            "title", "subject", "headline", "name", "label", "tag", "category", "role", "status",
            "state", "type", "action", "reason",
        ],
    ) {
